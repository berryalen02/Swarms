...
int kprobe__security_socket_connect(struct pt_regs *ctx)
{
    ...
    // sockaddr.sa_family 是u16位
    u16 address_family = 0;
    bpf_probe_read(&address_family, sizeof(address_family), &address->sa_family);
    // char fmt[] = "Address family: %u\n";
    // bpf_trace_printk(fmt, sizeof(fmt), address_family);
    ...
    if (address_family == AF_INET)
    {
        ...
        // 进程白名单匹配
        int check;
        check = task_whitelist_check(data4.task);
        if (check == 1){
            return 0;
        }
    ...
    else if (address_family == AF_INET6)
    {
        ...
        // 进程白名单匹配
        int check;
        check = task_whitelist_check(data6.task);
        if (check == 1){
            return 0;
        }
    ...
    { // other sockets, except UNIX and UNSPEC sockets
        ...
        // 进程白名单匹配
        int check;
        check = task_whitelist_check(socket_event.task);
        if (check == 1){
            return 0;
        }
